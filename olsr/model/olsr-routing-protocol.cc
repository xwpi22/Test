...
#define OLSR_WILL_NEVER0         0
/// Willingness for forwarding packets from other nodes: low.
#define OLSR_WILL_LOW1           1
#define OLSR_WILL_LOW2           2
/// Willingness for forwarding packets from other nodes: medium.
#define OLSR_WILL_DEFAULT3       3
/// Willingness for forwarding packets from other nodes: high.
#define OLSR_WILL_HIGH4          4
#define OLSR_WILL_HIGH5          5
#define OLSR_WILL_HIGH6          6
/// Willingness for forwarding packets from other nodes: always.
#define OLSR_WILL_ALWAYS7        7
...
extern double NodeToRemainingEnergy[100];

extern ns3::Ipv4Address NodeToAddress[100];
extern int numNodes;
namespace ns3 {
...
RoutingProtocol::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::olsr::RoutingProtocol")
    ...
    .AddAttribute ("Willingness", "Willingness of a node to carry and forward traffic for other nodes.",
                   EnumValue (OLSR_WILL_DEFAULT3),
                   MakeEnumAccessor (&RoutingProtocol::m_willingness),
                   MakeEnumChecker (OLSR_WILL_NEVER0, "never0",
                                    OLSR_WILL_LOW1, "low1",
                                    OLSR_WILL_LOW2, "low2",
                                    OLSR_WILL_DEFAULT3, "default3",
                                    OLSR_WILL_HIGH4, "high4",
                                    OLSR_WILL_HIGH5, "high5",
                                    OLSR_WILL_HIGH6, "high6",
                                    OLSR_WILL_ALWAYS7, "always7"))
...
RoutingProtocol::MprComputation  (void)
{
      ...
      for (NeighborSet::const_iterator neigh = N.begin ();
           neigh != N.end (); neigh++)
        {
          if (neigh->neighborMainAddr == twoHopNeigh->neighborMainAddr)
            {
              if (neigh->willingness == OLSR_WILL_NEVER0)
                {
                  ok = false;
                  break;
                }
  ...
  for (NeighborSet::const_iterator neighbor = N.begin (); neighbor != N.end (); neighbor++)
    {
      if (neighbor->willingness == OLSR_WILL_ALWAYS7)
        {
      ...
      for (NeighborSet::const_iterator neighbor = neighborSet.begin ();
           neighbor != neighborSet.end (); neighbor++)
        {
          if (neighbor->neighborMainAddr == nb2hop_tuple.neighborMainAddr
              && neighbor->willingness != OLSR_WILL_NEVER0)
            {
              nb2hopOk = true;
              break;
            }
                               ...
                               const Ipv4Address &senderIface)
{
  for(int i = 0; i<numNodes;i++){
    if(m_mainAddress == NodeToAddress[i])
      {
        if(NodeToRemainingEnergy[i] > 0.380){
          ...
          // std::cout<<m_mainAddress<<"doesn't have enough energy("<<NodeToRemainingEnergy[i]<<") to process hello!\n";
        }
      }
  }
}
    ...
    if(m_mainAddress == NodeToAddress[i])
      {
        if(NodeToRemainingEnergy[i] > 0.380){
          ...
          // std::cout<<m_mainAddress<<" doesn't have enough energy("<<NodeToRemainingEnergy[i]<<") to send packet!\n";
        }
      }
  }
...
RoutingProtocol::SendHello  (void)
{
  ...
  //remember to comment the for-loop below to simulate original OLSR
  for(int i=0; i<numNodes; i++){
    if(m_mainAddress == NodeToAddress[i]){
      double a = NodeToInitialEnergy[i];
      // std::cout << a << "\n";
      uint8_t will = uint8_t ('0' + (int) round (NodeToRemainingEnergy[i] / a * 7.));
      m_willingness = will;
      break;
      // std::cout<<m_mainAddress<<"'s willingness = "<<m_willingness<<"\n";
    }
  }
...
RoutingProtocol::NeighborLoss (const LinkTuple &tuple)
{
  ...
  LinkTupleUpdated (tuple, OLSR_WILL_DEFAULT3);
